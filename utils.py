...
class AddSpeakersToTranscrip:
    ...
    def add_speaker_info_to_text(self, timestamp_texts, ann, audio_chunk, sample_rate, chunk_number):
        ...
        for seg, text in timestamp_texts:
            ...
            if isinstance(embedding, torch.Tensor):
                ...
                raise TypeError(
                    f"Unexpected type for embedding: {type(embedding)}")
            ...
            if self.speaker_embeddings:
                print('INIT COSINE')
                distances = [cosine(embedding, np.ravel(emb))
                             for emb in self.speaker_embeddings.values()]
                min_distance = min(distances)
                if min_distance < 0.7:
                    matched_speaker = list(self.speaker_embeddings.keys())[
                        distances.index(min_distance)]
                    # Update the embedding (moving average)
                    self.speaker_embeddings[matched_speaker] = 0.9 * \
                        self.speaker_embeddings[matched_speaker] + \
                        0.1 * embedding
                    spk = matched_speaker
            ...
            adjusted_seg = Segment(
                seg.start + chunk_number * self.RECORD_SECONDS, seg.end + chunk_number * self.RECORD_SECONDS)
            spk_text.append((adjusted_seg, spk, text))
    ...
    def diarize_text(self, transcribe_res, diarization_result, audio_chunk, sample_rate, chunk_number):
        ...
        spk_text = self.add_speaker_info_to_text(
            timestamp_texts, diarization_result, audio_chunk, sample_rate, chunk_number)
